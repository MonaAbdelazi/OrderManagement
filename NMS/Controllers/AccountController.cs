...
namespace NMS.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        private Kafouri dbx = new Kafouri();
        ...
        public ActionResult Login(ManageMessageId? message)
       {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                ...
                catch (HttpAntiForgeryException ex)   //ex is HttpAntiForgeryException) { filterContext.Result = new RedirectResult("~/Account/Login"); }
                {
                    //if(ex is HttpAntiForgeryException)
                        ModelState.AddModelError("", "Kindly try again");
                    return RedirectToAction("Login", new { Message = ManageMessageId.Error });
                }
            }
        ...
        public ActionResult Register()
        {
            ...
            if (Session["Role"]!=null &&  Session["Role"].ToString() == "Admins")
            ViewBag.Roles = new SelectList(dbcx.Roles.ToList(), "Name", "Name");
            else
                ViewBag.Roles = new SelectList(getROLE(), "Value", "Text");

            return View("Register");
        }
        internal static List<SelectListItem> getROLE()
        {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
           ...
           //   db.SaveChanges();
                ...
                {
                        //AddErrors(result);
                       CommonUtils.SetFeedback(result.Errors.ToArray()[0].ToString(), Feedback.Feedback_Error);
                        string role = Session["Role"].ToString();
                        if (Session["Role"] != null && Session["Role"].ToString() == "Admins")
                            ViewBag.Roles = new SelectList(dbcx.Roles.ToList(), "Name", "Name");
                        else
